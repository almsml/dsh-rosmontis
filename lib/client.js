  ...
  function BigFishCard() {
    ...
    return React.createElement('li', { style: cardStyle, 'data-testid': 'dsh-dafeiyu-settings' },
      React.createElement('div', null,
        React.createElement('strong', { style: { fontSize: 16 } }, '迷迭香桌面伴侣'),
        React.createElement('p', { style: { margin: '5px 0 0', opacity: 0.72 } }, '入口和状态属于 DSH，迷迭香始终显示在 Windows 桌面最上层。'),
      ),
      status === 'unavailable'
        ? React.createElement('span', { role: 'status' }, '迷迭香设置尚未连接到 DSH Host。')
        : status === 'loading'
        ? React.createElement('span', null, '正在读取设置…')
        : React.createElement(React.Fragment, null,
          React.createElement(Field, { label: '启用迷迭香', hint: '关闭后立即退出；重新开启无需单独启动程序。' },
          ...
          React.createElement(Field, { label: '微动作时长', hint: `${Math.round(value.microMs ?? 1000)}ms，空闲微动作闪现时长` },
            React.createElement('input', {
              type: 'range', min: 400, max: 3000, step: 100, value: value.microMs ?? 1000,
              disabled: status !== 'ready',
              onChange: (event) => void writeSlider('microMs', Number(event.target.value)),
            }),
          ),
