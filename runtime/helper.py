...
def run_visual(recorder: EventRecorder, snapshot_path: Path | None = None) -> int:
    ...
    class CompanionWindow(QWidget):
        ...
        def _weighted_micro_index(self) -> int:
            ...
            less likely to repeat, and all weights recover toward 1.0 over time."""
            names = list(self.model.idle_micro_clips)
            if not names:
                return 0
            weights = self.micro_weights
            # recover every weight toward 1.0
            for name in weights:
                weights[name] = min(1.0, weights[name] + (1.0 - weights[name]) * 0.25)
            total = sum(weights.get(name, 1.0) for name in names)
            r = random.uniform(0.0, total)
            for i, name in enumerate(names):
                r -= weights.get(name, 1.0)
                if r <= 0:
                    weights[name] *= 0.3  # heavily decay the one just shown
                    return i
            return 0
